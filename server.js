...
server.get("/books", (req, res) => {
  ...
  const booksPlusRating = books.map((book) => {
    return {
      ...
      rating: book.reviews.length > 0 ? book.reviews.map(e => e.rating).reduce((acc, item) => acc + item) / book.reviews.length : 0.0,
    }
  });
...
server.get("/book/:index", (req, res) => {
  ...
  return res.json({
    ...
    rating: books[index].reviews.length > 0 ? books[index].reviews.map(e => e.rating).reduce((acc, item) => acc + item) / books[index].reviews.length : 0.0,
  });
});
